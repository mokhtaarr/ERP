...
namespace MSCustomer {
    ...
    var Customer: MS_Customer = new MS_Customer();
    var Customers: Array<MS_Customer> = new Array<MS_Customer>();
    var CustomerTypes: Array<Ms_CustomerTypes> = new Array<Ms_CustomerTypes>();
    var CustomerCategory: Array<MS_CustomerCategory> = new Array<MS_CustomerCategory>();
    ...
    ///////// Detailes ///////////////////////
    var Detailes: DetailesForCustomer = new DetailesForCustomer();

    var newBranche: Ms_CustomerBranches = new Ms_CustomerBranches();
    var CustomerBranches: Array<Ms_CustomerBranches> = new Array<Ms_CustomerBranches>();

    var AllUsersForDropdown: Array<CustomCusromerUsers> = new Array<CustomCusromerUsers>();
    var newUser: CustomCusromerUsers = new CustomCusromerUsers();
    var CustomerUsers: Array<CustomCusromerUsers> = new Array<CustomCusromerUsers>();
    var Users: Array<Ms_CusromerUsers> = new Array<Ms_CusromerUsers>();

    var newContact: Ms_CustomerContacts = new Ms_CustomerContacts();
    var CustomerContacts: Array<Ms_CustomerContacts> = new Array<Ms_CustomerContacts>();
    var newAccount: Cal_CustAccounts = new Cal_CustAccounts();
    var CustAccounts: Array<Cal_CustAccounts> = new Array<Cal_CustAccounts>();
    ...
    // select Options
    var CustomerTypeId: HTMLSelectElement;
    var CustomerCatId: HTMLSelectElement;
    ...
    var divCustomerGrid: JsGrid = new JsGrid();
    ...
    export function InitalizeComponent() {
        ...
        SharedButtons.PrintAction(() => { Disabled(true); });
    ...
    function InitalizeControls() {
        ...
        // select Options
        CustomerTypeId = document.getElementById("CustomerTypeId") as HTMLSelectElement;
        CustomerCatId = document.getElementById("CustomerCatId") as HTMLSelectElement;
        ...
        SharedButtons.btnSearch = document.getElementById("btnCustomerSearch") as HTMLButtonElement;
    }
    ...
    function InitalizeEvents() {
        ...
        SharedButtons.btnPrint.onclick = () => { PrintReport(1); };
    }
    ...
    function ClearGrids() {
        ClearBranchesGrid();
        CustomerBranches = new Array<Ms_CustomerBranches>();
        CustomerUsers = new Array<CustomCusromerUsers>();
        Users = new Array<Ms_CusromerUsers>();
        CustomerContacts = new Array<Ms_CustomerContacts>();
    }

    function ClearBranchesGrid() {
        var ClrCustomerBranches = new Array<Ms_CustomerBranches>();
        divBranchesGrid.DataSource = ClrCustomerBranches;
        divBranchesGrid.Bind();

        var ClrCustomerUsers = new Array<CustomCusromerUsers>();
        divUsersGrid.DataSource = ClrCustomerUsers;
        divUsersGrid.Bind();

        var ClrCustomerContacts = new Array<Ms_CustomerContacts>();
    ...
    function GetAll() {
        ...
        Ajax.Callsync({
            ...
            success: (d) => {
                ...
                if (result.IsSuccess) {
                    Customers = result.Response as Array<MS_Customer>;
    ...
    function GetById(customerId: number) {
        Ajax.Callsync({
            type: "Get",
            ...
            success: (d) => {
                let result = d as BaseResponse;
                if (result.IsSuccess) {
                    Detailes = result.Response as DetailesForCustomer;
                }
            }
        });
        return Detailes;
    ...
    function InitializeGrid() {
        ...
        divCustomerGrid.OnItemInserting = () => { };
        divCustomerGrid.OnItemUpdating = () => { };
        divCustomerGrid.OnItemDeleting = () => { };
        divCustomerGrid.OnRowSelected = () => {
            GetDataRow();
            SharedWork.SwitchModes(ScreenModes.Query);
        };
        divCustomerGrid.OnRowDoubleClicked = () => {
            GetDataRow();
            SharedWork.SwitchModes(ScreenModes.Query);
        }
        divCustomerGrid.Columns = [
            ...
            },
            {
                ...
                headerTemplate: (): HTMLElement => {
                    let txt = CreateElement("text", GridInputClassName, " ", " ", "CustomerId", " ");
                    txt.disabled = true;
    ...
    function GetDataRow() {
        var obj: MS_Customer = divCustomerGrid.SelectedItem;
    ...
    function Insert() {
        ...
        Ajax.Callsync({
            ...
            success: (Response) => {
                ...
                if (result.IsSuccess) {
                    Customer = result.Response as MS_Customer;
                    _CustomerId = Customer.CustomerId;
                    Success = true;
                }
                else {
    ...
    function GetCustomerCategory() {
        Ajax.Callsync({
            ...
            success: (Response) => {
                let result = Response as BaseResponse;
                if (result.IsSuccess) {
                    CustomerCategory = result.Response as Array<MS_CustomerCategory>;
    ...
    function btnSearch_onclick() {
        ...
        sys.FindKey(Modules.MS_Customer, SharedButtons.btnSearch.id, "", () => {
    ...
    function GetAllUsers() {
        Ajax.Callsync({
            ...
            success: (Response) => {
                AllUsersForDropdown = Response as Array<CustomCusromerUsers>;
            }
        });
    }
    ...
    function GetUser(code: string) {
        Ajax.Callsync({
            ...
            success: (Response) => {
                let result = Response as BaseResponse;
                if (result.IsSuccess) {
                    newUser = result.Response as CustomCusromerUsers;
    ...
    function fillUserInput(User: CustomCusromerUsers) {
    ...
    function FillDetailes(Detailes: DetailesForCustomer) {
    ...
    function AddCustBranchInGrid() {
        ...
        let hd_CustBranchCode = $('#hd_CustBranchCode'),
            hd_CustBranchName1 = $('#hd_CustBranchName1'),
            hd_CustBranchId = $('#hd_CustBranchId'),
            hd_CustomerId = $('#hd_CustomerId');

        if (hd_CustBranchCode.val().trim() == "") { MessageBox.Show(Resource.PleaseEnterCode, Resource.Error); flag = false; return }
        else newBranche.CustBranchCode = hd_CustBranchCode.val().trim();

        if (hd_CustBranchName1.val().trim() == "") { MessageBox.Show(Resource.PleaseEnterName + " 1", Resource.Error); flag = false; return }
        else newBranche.CustBranchName1 = hd_CustBranchName1.val().trim();
        ...
        if (flag) {
            ...
            if (hd_CustBranchId.val().trim() == "") newBranche.CustBranchId = 0;
            else newBranche.CustBranchId = hd_CustBranchId.val().trim();
        ...
        divBranchesGrid.Columns = [
            {
                ...
                itemTemplate: (s: string, item: Ms_CustomerBranches): HTMLButtonElement => {
                    let btn: HTMLButtonElement = DocumentActions.CreateElement<HTMLButtonElement>("button");
            ...
            ,
            {
                css: JsGridHeaderCenter, itemTemplate: (s: string, item: Ms_CustomerBranches): HTMLButtonElement => {
                    let btn: HTMLButtonElement = DocumentActions.CreateElement<HTMLButtonElement>("button");
                    ...
                    btn.onclick = (e) => {
                        ...
                        divBranchesGrid.DataSource = CustomerBranches;
                        divBranchesGrid.Bind();
                        if (item.CustBranchCode != null) FillInputText("hd_CustBranchCode", item.CustBranchCode.toString());
                        if (item.CustBranchName1 != null) FillInputText("hd_CustBranchName1", item.CustBranchName1.toString());
                        if (item.CustBranchName2 != null) FillInputText("hd_CustBranchName2", item.CustBranchName2.toString());
                        ...
                        if (item.CustBranchId != null) FillInputText("hd_CustBranchId", item.CustBranchId.toString());
                        if (item.CustomerId != null) FillInputText("hd_CustomerId", item.CustomerId.toString());
            ...
            {
                title: Resource.BranchCode, css: "ColumPadding", name: "CustBranchCode", headerTemplate: (): HTMLElement => {
                    let txt = CreateElement("text", GridInputClassName, " ", " ", "CustBranchCode", " ");
                    txt.id = "hd_CustBranchCode";
                    return HeaderTemplate(Resource.BranchCode, txt);
                }
            },
            {
                title: Resource.App_Name + " 1", css: "ColumPadding", name: "CustBranchName1", headerTemplate: (): HTMLElement => {
                    let txt = CreateElement("text", GridInputClassName, " ", " ", "CustBranchName1", " ");
                    txt.id = "hd_CustBranchName1";
                    return HeaderTemplate(Resource.App_Name + " 1", txt);
                }
            },
            {
                title: Resource.App_Name + " 2", css: "ColumPadding", name: "CustBranchName2", headerTemplate: (): HTMLElement => {
                    let txt = CreateElement("text", GridInputClassName, " ", " ", "CustBranchName2", " ");
            ...
            },
            {
                ...
                headerTemplate: (): HTMLElement => {
                    let txt = CreateElement("text", GridInputClassName, " ", " ", "CustBranchId", " ");
                    txt.disabled = false;
    ...
    function AddUsersGridInGrid() {
        ...
        if (flag) {
            ...
            if (hd_CustUserId.val().trim() == "") newUser.CustUserId = 0;
            else newUser.CustUserId = hd_CustUserId.val().trim();
            if (hd_UserId.val().trim() == "") newUser.UserId = 0;
            else newUser.UserId = hd_UserId.val().trim();
            if (hd_CustomerId.val().trim() == "") newUser.CustomerId = 0;
            else newUser.CustomerId = hd_CustomerId.val().trim();
        ...
        divUsersGrid.Columns = [
            {
                ...
                itemTemplate: (s: string, item: CustomCusromerUsers): HTMLButtonElement => {
                    let btn: HTMLButtonElement = DocumentActions.CreateElement<HTMLButtonElement>("button");
                    btn.innerHTML = "<i class='fas fa-times'></i>";
                    btn.className = TransparentButton + "  red_Delete_Cotnrol editable";
            ...
            {
                css: JsGridHeaderCenter, itemTemplate: (s: string, item: CustomCusromerUsers): HTMLButtonElement => {
                    let btn: HTMLButtonElement = DocumentActions.CreateElement<HTMLButtonElement>("button");
                    btn.innerHTML = "<i class='fa fa-edit'></i>";
                    btn.className = TransparentButton + " " + "emptrainingedit " + "green_edit_control editable";
                    ...
                    btn.onclick = (e) => {
                        ...
                        if (item.CustUserId != null) FillInputText("hd_CustUserId", item.CustUserId.toString());
                        if (item.CustomerId != null) FillInputText("hd_CustomerId", item.CustomerId.toString());
                        FillInputText("hd_Flag", "u");
                    };
                    return btn;
            ...
            {
                title: "CustomerId", css: "ColumPadding hide", name: "CustomerId", width: "1%", headerTemplate: (): HTMLElement => {
                    let txt = CreateElement("text", GridInputClassName, " ", " ", "CustomerId", " ");
                    txt.disabled = false;
                    txt.id = "hd_CustomerId";
                    return HeaderTemplate("CustomerId", txt);
            ...
            {
                title: "CustUserId", css: "ColumPadding hide", name: "CustUserId", width: "1%", headerTemplate: (): HTMLElement => {
                    let txt = CreateElement("text", GridInputClassName, " ", " ", "CustUserId", " ");
                    txt.disabled = false;
    ...
    function AddContactsGridInGrid() {
        ...
        let hd_ContactCode = $('#hd_ContactCode'),
            ...
            hd_CustContactId = $('#hd_CustContactId'),
            hd_CustomerId = $('#hd_CustomerId');

        if (hd_ContactCode.val().trim() == "") { MessageBox.Show(Resource.PleaseEnterCode, Resource.Error); flag = false; return }
        else {
            if (!CheckCode(CustomerContacts, hd_ContactCode.val(), "ContactCode")) { MessageBox.Show(Resource.CodeCannotDuplicated, Resource.Error); flag = false; return }
        ...
        if (flag) {
            ...
            if (hd_CustContactId.val().trim() == "") newContact.CustContactId = 0;
            else newContact.CustContactId = hd_CustContactId.val().trim();
            if (hd_CustomerId.val().trim() == "") newContact.CustomerId = 0;
            else newContact.CustomerId = hd_CustomerId.val().trim();
        ...
        divContactsGrid.Columns = [
            {
                ...
                itemTemplate: (s: string, item: Ms_CustomerContacts): HTMLButtonElement => {
                    let btn: HTMLButtonElement = DocumentActions.CreateElement<HTMLButtonElement>("button");
                    btn.innerHTML = "<i class='fas fa-times'></i>";
                    btn.className = TransparentButton + "  red_Delete_Cotnrol editable";
            ...
            {
                css: JsGridHeaderCenter, itemTemplate: (s: string, item: Ms_CustomerContacts): HTMLButtonElement => {
                    let btn: HTMLButtonElement = DocumentActions.CreateElement<HTMLButtonElement>("button");
                    btn.innerHTML = "<i class='fa fa-edit'></i>";
                    btn.className = TransparentButton + " " + "emptrainingedit " + "green_edit_control editable";
                    ...
                    btn.onclick = (e) => {
                        ...
                        if (item.CustContactId != null) FillInputText("hd_CustContactId", item.CustContactId.toString());
                        if (item.CustomerId != null) FillInputText("hd_CustomerId", item.CustomerId.toString());
                        FillInputText("hd_Flag", "u");
                    };
                    return btn;
            ...
            {
                title: "CustContactId", css: "ColumPadding hide", name: "CustContactId", width: "1%", headerTemplate: (): HTMLElement => {
                    let txt = CreateElement("text", GridInputClassName, " ", " ", "CustContactId", " ");
                    txt.disabled = false;
    ...
    function MapCustomAccounts() {
        CustAccounts = new Array<Cal_CustAccounts>();
