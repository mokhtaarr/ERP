$(document).ready(function () {
    ...
    MsReceiptNote.InitalizeComponent();
});
var MsReceiptNote;
(function (MsReceiptNote) {
    var Resource = GetResourceList("");
    $('#headertop1').addClass('display_none');
    $('#headertop2').removeClass('display_none');
    $('#headerTitle').text(Resource.ReceiptDocument);
    ...
    var Model = new Ms_ReceiptNote();
    var AllReceiptNoteIds = new Array();
    var ReceiptNoteCurrencies = new Array();
    var CurrencyCategories = new Array();
    var Details = new ReceiptNoteAndDetails();
    var Terms = new Array();
    var Books = new Array();
    var Boxs = new Array();
    var Currencies = new Array();
    //select Options
    var element, Rate, TermId, BookId, BankNoticId, ClientBankId, RectSourceType, BoxId, CurrencyId, CustomerId, EmpId, TranType, BankAccNumber, ClientAccNo, VendorId, SelectMode_RectSourceType = [], Total = 0, TotalCurancy = 0, ObjectId = 0, StatusFlag, Success, flag = true, CuranciesGridId = new JsGrid();
    function InitalizeComponent() {
        localStorage.setItem("TableName", "Ms_ReceiptNote");
    ...
    MsReceiptNote.InitalizeComponent = InitalizeComponent;
    function InitalizeControls() {
        SharedButtons.btnSearch = document.getElementById("btnMs_ReceiptNoteSearch");
        ...
        TranType = document.getElementById("TranType");
        BankNoticId = document.getElementById("BankNoticId");
        ClientBankId = document.getElementById("ClientBankId");
        ClientAccNo = document.getElementById("ClientAccNo");
        BankAccNumber = document.getElementById("BankAccNumber");
    ...
    function InitalizeEvents() {
        ...
        ClientBankId.onchange = function () {
            if (!IsNullOrEmpty(ClientBankId.value))
                GetBankAccounts(Number(ClientBankId.value));
        };
    }
    function GetAll() {
        Disabled(false);
        Ajax.Callsync({
            type: "Get",
            url: sys.apiUrl("Ms_ReceiptNote", "GetAll"),
            success: function (d) {
                var result = d;
                if (result.IsSuccess) {
                    AllReceiptNoteIds = result.Response;
                }
            }
        });
    }
    function GetByID(Id) {
        Ajax.Callsync({
            type: "Get",
            url: sys.apiUrl("Ms_ReceiptNote", "GetById"),
            data: { id: Id },
            success: function (d) {
                var result = d;
                if (result.IsSuccess) {
                    ...
                    if (res != null) {
                        ...
                        Display(res);
                        if (res.TranType == 2 && res.BankNoticId != null)
                            GetBankNotic(res.BankNoticId);
    ...
    function Assign() {
        Details.Ms_Receipt = DocumentActions.AssignToModel(Model);
    ...
    function Insert() {
        ...
        Ajax.Callsync({
            type: "POST",
            url: sys.apiUrl("Ms_ReceiptNote", "Insert"),
    ...
    function Update() {
        ...
        Ajax.Callsync({
            type: "POST",
            url: sys.apiUrl("Ms_ReceiptNote", "Update"),
            data: JSON.stringify(Details),
            success: function (Response) {
                var result = Response;
                if (result.IsSuccess) {
    ...
    function Delete() {
        ...
        Ajax.Callsync({
            type: "Get",
            url: sys.apiUrl("Ms_ReceiptNote", "Delete"),
            data: { id: Model.RectId },
    ...
    function Navigate() {
        Model = AllReceiptNoteIds[SharedWork.PageIndex - 1];
        ObjectId = Model.RectId;
        GetByID(ObjectId);
    }
    MsReceiptNote.Navigate = Navigate;
    function Display(model) {
        Model = model;
        Model.ChequeOpenId = Model.ChequeOpenId ? 1 : 0;
        Model.CheckType = Model.CheckType ? 1 : 0;
        DocumentActions.RenderFromModel(Model);
        ObjectId = Number(Model.RectId);
    }
    function Undo() {
        Disabled(false);
        Success = false;
    ...
    function EnabledOrDisabledSelectMode_RectSourceType(disabled) {
    ...
    function GetModel(id) {
        Model = AllReceiptNoteIds.filter(function (x) { return x.RectId == id; })[0];
        return Model;
    }
    ...
    function InitializeGrid() {
        ...
        CuranciesGridId.OnRowSelected = function () {
        };
        CuranciesGridId.Columns = [
            ...
            {
                title: Resource.value, css: "ColumPadding", name: "Value", itemTemplate: function (e) {
                    var val = e;
                    return val.toFixed(6);
                }
            },
            {
                title: Resource.UnitsCount, css: "ColumPadding", name: "Count", itemTemplate: function (e) {
                    var val = e;
                    return val.toFixed(6);
                }
            },
            {
                title: Resource.Price, css: "ColumPadding", name: "Price", itemTemplate: function (e) {
                    var val = e;
                    return val.toFixed(6);
                }
            },
    ...
    function ClearGrid() {
        CuranciesGridId.DataSource = new Array();
        CuranciesGridId.Bind();
    }
        ...
        Ajax.Callsync({
            ...
            success: function (Response) {
                ...
                if (result.IsSuccess) {
                    ...
                    DocumentActions.FillCombowithCode(Boxs, ClientBankId, "BoxId", (language == "ar" ? "DESCA" : "DESCE"), "BoxCode", Resource.EntityBankCode);
                }
            }
        });
        return Boxs;
    }
    function GetAllBanksEntiity() {
        Ajax.Callsync({
            type: "Get",
            url: sys.apiUrl("funcationShared", "GetAllBanks"),
            success: function (Response) {
    ...
    function ResetSafe(tranTypeVal) {
        if (SharedWork.CurrentMode == ScreenModes.Add || SharedWork.CurrentMode == ScreenModes.Edit) {
    ...
    function GetAllCurrencyCategory(currencyId) {
        Ajax.Callsync({
            type: "Get",
            ...
            success: function (Response) {
                var result = Response;
                if (result.IsSuccess) {
                    CurrencyCategories = result.Response;
    ...
    function GetRate(currencyId) {
        var Currency = Currencies.filter(function (x) { return x.CurrencyId == currencyId; })[0];
    ...
    function CalculationCurrency() {
        var ValueBeforeRate = 0, ValueAfterRate = 0, rate = Number(Rate.value), Price = Number($('#ValueBeforeRate').val());
        if (rate > 0 && Price > 0) {
    ...
    function CalculationSalaryBefor() {
        var rate = Number(Rate.value), countInput = $('.SalaryBefor').length;
        for (var i = 1; i <= countInput; i++) {
            var BeforeRate = $('#Value' + i + "BeforeRate").val(), afterrate = Number(BeforeRate) * rate;
            $('#Value' + i).val(afterrate);
        }
        SetTotalAndTotalCurancy();
    }
    function SetTotalAndTotalCurancy(ValueBeforeRate, PaidPrice) {
        if (ValueBeforeRate === void 0) { ValueBeforeRate = 0; }
        if (PaidPrice === void 0) { PaidPrice = 0; }
        var total = 0 + PaidPrice, totalCurancy = 0 + ValueBeforeRate, countInput = $('.SalaryBefor').length;
    ...
    function ResetCheck(tranTypeVal) {
        if (tranTypeVal === void 0) { tranTypeVal = 1; }
        if (SharedWork.CurrentMode == ScreenModes.Add || tranTypeVal == 1)
            $("#home :input").val('');
        $("#home select").each(function (index, element) {
            $('#' + element.id).select2().trigger('change');
        });
        if (tranTypeVal == 1 || tranTypeVal == 0) {
            $("#home :input").not("#RefNo,#DocTrNo").prop("disabled", true);
        }
        else {
            $("#home :input").not("#RefNo,#DocTrNo").prop("disabled", false);
        }
    }
    function DisabledOtherMoney() {
        var countInputs = $('.SalaryBefor').length;
        for (var i = 1; i <= countInputs; i++) {
            $('#Value' + i).prop("disabled", true);
        }
    }
    function GetBankAccounts(BoxId, IsBankNoticId) {
        if (IsBankNoticId === void 0) { IsBankNoticId = false; }
        Ajax.Callsync({
            type: "Get",
            ...
            success: function (Response) {
                ...
                if (result.IsSuccess) {
                    ...
                    DocumentActions.FillCombowithdefult(result.Response, (IsBankNoticId ? BankAccNumber : ClientAccNo), "AcountCode", (language == "ar" ? "AcounntNameA" : "AcounntNameE"), (IsBankNoticId ? Resource.AccountNo : Resource.EntityAccountNumber));
                }
            }
        });
    }
    function GetBankNotic(id) {
        Ajax.Callsync({
            type: "Get",
            ...
            success: function (Response) {
                ...
                var result = Response;
                if (result.IsSuccess) {
                    $('#RefNo').val((_a = result.Response) === null || _a === void 0 ? void 0 : _a.RefNo);
                    $('#DocTrNo').val((_b = result.Response) === null || _b === void 0 ? void 0 : _b.DocTrNo);
                }
            }
        });
    }
    function btnSearch_onclick() {
        var sys = new SystemTools();
        sys.FindKey(Modules.Ms_ReceiptNote, SharedButtons.btnSearch.id, "", function () {
...
})(MsReceiptNote || (MsReceiptNote = {}));
//# sourceMappingURL=Ms_ReceiptNote.js.map