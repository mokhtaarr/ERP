...
(function (Login_) {
    ...
    var Connection = new ConnectionObj();
    ...
    var InitialCatalog = document.getElementById("InitialCatalog");
    var Database = document.getElementById("Database");
    var Servers = Array();
    var Databases = Array();
    var GridConnection = new JsGrid();
    ...
    function LoadUserAuthentications(userId) {
        Ajax.Callsync({
            type: "Get",
    ...
    function InitializeGrid() {
        ...
        GridConnection.OnRowSelected = function () { };
        GridConnection.Columns = [
            ...
            {
                title: "ConnectionNumber", css: "ColumPadding hide", name: "ConnectionNumber", headerTemplate: function () {
                    var txt = CreateElement("text", GridInputClassName, " ", " ", "ConnectionNumber", " ");
                    txt.disabled = false;
    ...
    Login_.InitializeGrid = InitializeGrid;
    $('#Enter').on('click', function () {
        var datbase = $('#Database').val();
        var url = $('#GetAPIUrl').val();
        if (IsNullOrEmpty(datbase)) {
            MessageBox.Toastr("من فضلك اختر قاعدة البيانات", "خطأ", ToastrTypes.error);
            return;
        }
        Connection = Connections.filter(function (x) { return x.InitialCatalog == datbase; })[0];
        SaveConn(url);
    });
    function SaveConn(url) {
        Ajax.Callsync({
            type: "GET",
    ...
    $('#saveConnectionString').on('click', function () {
        var DataSource = $('#ServerName').val(), InitialCatalog = $('#InitialCatalog').val(), UserName = $('#DbUserName').val(), mode = $('#mode').val(), Password = $('#DbPassword').val(), IntegratedSecurity = $('#IntegratedSecurity').is(":checked");
    ...
    $('#ServerName,#InitialCatalog,#DbUserName,#DbPassword,#IntegratedSecurity').on('change', function () {
        var DataSource = $('#ServerName').val(), InitialCatalog = $('#InitialCatalog').val(), UserName = $('#DbUserName').val(), Password = $('#DbPassword').val(), IntegratedSecurity = $('#IntegratedSecurity').is(":checked");
        GetConnection(DataSource, InitialCatalog, IntegratedSecurity, UserName, Password);
    });
    var connection = "";
    function GetConnection(DataSource, InitialCatalog, IntegratedSecurity, UserID, Password) {
    ...
    function GetConectionFromGrid(value) {
        $('#mode').val('edit');
        Connection = Connections.filter(function (x) { return x.ConnectionNumber == value; })[0];
    ...
    function SetDatabasesFromConnections(connections) {
        if (connections != null) {
            var Databases_1 = connections.map(function (a) { return a.InitialCatalog; });
            DocumentActions.FillCombowithdefultListOneValue(Databases_1, Database, "", "", "اختر قاعدة البيانات");
        }
    }
